...
uint8_t halRfReadReg(uint8_t spi_instr)
{
	...
	// errata Section 3. You have to make sure that you read the same value of 
	// the register twice in a row before you evaluate it 
	// otherwise you might read a value that is a mix of 2 state values.
	rbuf[0] = spi_instr | READ_SINGLE_BYTE;
	rbuf[1] = 0;
	wiringPiSPIDataRW (0, rbuf, len) ;
...
{   
	...
	// channel 0 100khz   min 500khz ds la doc ?
	if ( (wiringPiSPISetup (0, 100000)) < 0 )  {
		//fprintf (stderr, "Can't open the SPI bus: %s\n", strerror (errno)) ;
		printf ("wiringPiSPISetup : Can't open the SPI bus");
		exit (EXIT_FAILURE) ;
	}  
	cc1101_reset();
	delay(1); //1ms
...
bool echo_cc1101_version(bool show)
{
	...
	if (show) {
		printf("CC1101 Verion : 0x%02X%02X\r\n", halRfReadReg(PARTNUM_ADDR), version );
	}
	// != 00 or 0xFF
...
{	
	...
	while ((l_byte_in_rx == 0) && (l_tmo<rx_tmo_ms)) {
		...
		if (l_byte_in_rx) {
			...
			if ( debug_out ) {
				show_in_hex_one_line(rxBuffer,l_byte_in_rx);
			}
		}
	}

	if ( l_tmo<rx_tmo_ms ) {
		echo_debug(debug_out,"1st synch received "); 
	} else { 
		return 0;
	}
	...
	if ( l_tmo<rx_tmo_ms ) {
		echo_debug(debug_out,"frame received\r\n"); 
	} else { 
		return 0;
	}
